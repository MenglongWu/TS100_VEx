...
*		*** 该函数现在已经不用，它属于精简版的pow函数，后期的代码用库函数pow代替 ***
*		但是理论基础不变，详情看下面Remark里的例子，
*		例子里关于-10dBm的校准值ADC=350、DAC300保存在FLASH里，整个系统只有一个
*		校准点

*		该算法的特点，将-10dBm看成归零点，而不是0dBm，计算-7dBm时看成计算一个比
*		-10dBm大3dBm的数值，这样c10_xxx这些数组只要记录正数，而不用记录负数，减少
*		内存使用，并且c10_xxx数组能保证满足10^9 ~ 10^0.0001的计算精度
...
****************************************************************
*	功率控制
*	读取FLASH里的-10dBm ADC和DAC的校准值，从而计算用于预设置功率值，
*	计算精度是0.001dbm
* Parameters:
	传递参数结构
	-1dBm = v->set = -1000;
...
/*
* Function: 进入后台校准界面
  后后台密码：切换光输出模式，切换到CW时表示输入一个密码，
  密码为-9dbm,-8dbm,-7dbm,-4dbm
*/
void IsHacker()
{
